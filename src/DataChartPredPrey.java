 ...
 * @author timmolter
 */
public class DataChartPredPrey {
    static final int WIDTH = 1000;
    static final int HEIGHT = 500;

    public static void main(String[] args) {
        DataChartPredPrey predPreyChart = new DataChartPredPrey();
    ...
    public void drawChart() {
        ...
        List<List> allSeries = importCSV("resources/predprey.csv");
        ...
        // Space tick marks on x-axis
        styler.setXAxisMax(((double) xData.get(xData.size() - 1) + 10));
        // Space tick marks on y-axis
       ...
       // chart.getStyler().setYAxisGroupPosition(1, YAxisPosition.Right);
        styler.setYAxisGroupPosition(1, YAxisPosition.Right);
        charts.add(chart);
        new SwingWrapper(charts).displayChartMatrix();
    }

    public List<List> importCSV(String filePath) {
        List<List> allSeries = new ArrayList<>();
        List<Integer> xData = new ArrayList<>();
        List<Integer> yData = new ArrayList<>();
        List<Integer> yData1 = new ArrayList<>();
        //  Add the three series to a List of lists, these are populated below.
        allSeries.add(xData);
        allSeries.add(yData);
